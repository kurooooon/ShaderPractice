Shader "Custom/FragmentTextureGlass01" {
	Properties {
	...
	SubShader {
		Pass {
			...
			vertout vert (appdata_full v) {
				...
				OUT.TangentRX = mul(rotation, _Object2World[0].xyz);
				OUT.TangentRY = mul(rotation, _Object2World[1].xyz);
				OUT.TangentRZ = mul(rotation, _Object2World[2].xyz);
			...
			fixed4 frag (vertout In) :COLOR {
				...
				float4 reflectVect;
				reflectVect.xyz = reflect(-In.viewDir, normal);
				reflectVect.w = 1;
				float4 reflectColor = texCUBE(_Cubemap, reflectVect);
				float4 refractVect;
				refractVect.xyz = refract(In.viewDir, normal, 0.667);
				refractVect.w = 1;
				float4 refractColor = texCUBE(_Cubemap, refractVect);
