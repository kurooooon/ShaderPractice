Shader "Custom/FragmentTextureRefl01" {
	Properties {
	...
	SubShader {
		Pass {
			...
			fixed4 frag (vertout In) :COLOR {
				...
				reflectVect.w = 1;
				float Emission = texCUBE(_Cubemap, reflectVect);
				float4 color = UNITY_LIGHTMODEL_AMBIENT + 
					(_Diffuse * _LightColor0 * diffuse + _LightColor0 * half4(1.0, 1.0, 1.0, 1.0) * specular) * atten + Emission;
				return color;
			}
			ENDCG
		}
